...
function displayUsage() {
	global $log, $port, $address, $socket_id;
  ...
  echo "\t\t--id=<id> Database id related with this socket (default '$socket_id')\n";
	echo "\n";
} //end display usage
		...
		foreach($changed as $socket) {
			if($socket == $master) {
				...
				if($client < 0) { logToFile("socket_accept() failed");
					...
					WsConnect($client, $sockets, $users, $socket_id);
